...
public class ChatsController: UIViewController, UITableViewDelegate, UITableViewDataSource {
  ...
  public override func viewDidLoad() {
    ...
    BotStacksChatStore.current.objectWillChange.makeConnectable().autoconnect().sink { [weak self] _ in
  ...
  var dmsUnreadCount: Int {
    return BotStacksChatStore.current.dms.reduce(0, {$1.unreadCount + $0})
  }
  var groupsUnreadCount: Int {
    return BotStacksChatStore.current.groups.reduce(0, {$1.unreadCount + $0})
  }
  ...
  var chats: [Chat] {
    return isChannels ? BotStacksChatStore.current.groups : BotStacksChatStore.current.dms
  }
...
public struct ChatsView: View {
  ...
  @State var list: BotStacksChatStore.List = .groups
  ...
  public var body: some View {
    ScrollViewReader { proxy in
      ZStack(alignment: .topLeading) {
        ...
        let header = {
          HStack(spacing: 0) {
            ForEach(BotStacksChatStore.List.all, id: \.rawValue) {
              ChatTabView(
                current: $list,
                tab: $0,

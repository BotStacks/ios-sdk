...
public class UIInviteUsers: UIBaseController, UITableViewDelegate, UITableViewDataSource {
  ...
  func loadUsers() -> [User] {
    var users = BotStacksChatStore.current.contacts.items
    if let chat = chat {
...
//    btnBackBottom.titleLabel?.textColor = Theme.current.inverted.text.ui
    ...
    BotStacksChatStore.current.contacts.objectWillChange.makeConnectable()
      .autoconnect()
      .sink { [weak self] _ in
  ...
  public func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
    ...
    BotStacksChatStore.current.contacts.loadMoreIfNeeded(user)
  }

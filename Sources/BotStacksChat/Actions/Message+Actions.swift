extension Message {
  ...
  func react(_ reaction: String) {
    ...
    if action != .delete {
      BotStacksChatStore.current.onReaction(reaction)
    }
    Task.detached {
      do {
  ...
  func delete() {
    ...
    BotStacksChatStore.current.cache.messages.removeValue(forKey: self.id)
    Task.detached {
      do {
  ...
  func hide() {
    if !user.isCurrent {
      BotStacksChatStore.current.hiddenMessages.insert(self.id)

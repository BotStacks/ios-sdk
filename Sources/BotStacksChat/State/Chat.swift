...
public final class Chat: Pager<Message>, Identifiable {
  ...
  override public func load(skip: Int, limit: Int) async -> [Message] {
    do {
      let items = (try await api.fetchMessages(
        self.id, skip: skip, limit: limit)).filter({!BotStacksChatStore.current.hiddenMessages.contains($0.id) && !BotStacksChatStore.current.hiddenUsers.contains($0.userID)})
      return items
    } catch let err {
  ...
  var isInvited: Bool {
   ...
   init(
    ...
    super.init()
    BotStacksChatStore.current.cache.chats[id] = self
    if (kind == Gql.ChatType.directMessage), let friend = self.friend {
      BotStacksChatStore.current.cache.chatsByUID[friend.id] = self
    }
  }
  ...
  static func get(_ chat: Gql.FChat) -> Chat {
    if let g = BotStacksChatStore.current.cache.chats[chat.id] {
      g.update(chat)
      return g
    }
    return Chat(chat)
  ...
  static func get(_ id: String) -> Chat? {
    return BotStacksChatStore.current.cache.chats[id]
  }
  
  static func get(uid: String) -> Chat? {
    return BotStacksChatStore.current.cache.chatsByUID[uid]
  }
  
  static func fetch(id: String) {
    if BotStacksChatStore.current.cache.chatFetches.contains(id) { return }
    BotStacksChatStore.current.cache.chatFetches.insert(id)
    Task.detached {
      do {

...
public struct Route<ValidatedData, Content: View>: View {
  ...
  typealias Validator = (RouteInformation) -> ValidatedData?

  @Environment(\.relativePath) private var relativePath
  @Environment(\.path) private var currentPath
...
extension Route where ValidatedData == RouteInformation {
  ...
  public init(_ path: String = "*", @ViewBuilder content: @escaping (RouteInformation) -> Content) {
    self.path = path
    self.validator = { $0 }
    self.content = content
  }
  ...
  public init(_ path: String = "*", content: @autoclosure @escaping () -> Content) {
    self.path = path
    self.validator = { $0 }
    self.content = { _ in content() }
  }
...
public final class PathMatcher: ObservableObject {
  ...
  private static let variablesRegex = try! NSRegularExpression(
    pattern: #":([^\/\?]+)"#, options: [])
  ...
  private func compileRegex(_ glob: String) throws -> CompiledRegex {
    ...
    for match in variableMatches where match.numberOfRanges > 1 {
      if let range = Range(match.range(at: 1), in: glob) {
        let variable = String(glob[range])
          ...
          if let r = variable.range(
            of: "(^[^a-z]|[^a-z0-9])", options: [.regularExpression, .caseInsensitive])
          {
    ...
    pattern = "^" + (pattern.isEmpty ? "" : "(\(pattern))") + (endsWithAsterisk ? "(/.*)?$" : "$")
  ...
  func match(glob: String, with path: String, relative: String = "/") throws -> RouteInformation? {
    ...
    return RouteInformation(
      path: resolvedGlob, matchedPath: matchedPath, parameters: parameterValues)
  }
}

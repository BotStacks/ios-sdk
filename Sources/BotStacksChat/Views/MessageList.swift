...
public class UIMessageList: UIViewController, UITableViewDelegate, UITableViewDataSource {
  ...
  var pager: Pager<Message> {
    return chat ?? BotStacksChatStore.current.favorites
  }
  ...
  var messages = BotStacksChatStore.current.favorites.items {
    didSet {
  ...
  override public func viewDidLoad() {
    ...
    if chat != nil {
      ...
      BotStacksChatStore.current.favorites.objectWillChange.makeConnectable()
        .autoconnect()
        .throttle(for: 0.1, scheduler: RunLoop.main, latest: true)
        .sink(receiveValue: { [weak self] _ in
          DispatchQueue.main.async {

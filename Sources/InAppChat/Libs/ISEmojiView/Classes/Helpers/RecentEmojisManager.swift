...
final public class RecentEmojisManager {
    ...
    public func add(emoji: ISEmoji, selectedEmoji: String, maxCount: Int) -> Bool {
        if (maxCount == 0) {
    ...
    public func recentEmojis() -> [ISEmoji] {
        guard let data = UserDefaults.standard.data(forKey: recentEmojisKey) else {
            return []
        }
        
        guard let emojis = try? JSONDecoder().decode([ISEmoji].self, from: data) else {
            return []
        }

...
internal protocol EmojiCollectionViewDelegate: AnyObject {
    ...
    func emojiViewDidSelectEmoji(emojiView: EmojiCollectionView, emoji: ISEmoji, selectedEmoji: String)
...
internal class EmojiCollectionView: UIView {
    ...
    internal func updateRecentsEmojis(_ emojis: [ISEmoji]) {
...
extension EmojiCollectionView {
    ...
    private func dismissPopView(_ usePopViewEmoji: Bool) {
        ...
        if !currentEmoji.isEmpty && usePopViewEmoji {
            self.delegate?.emojiViewDidSelectEmoji(emojiView: self, emoji: ISEmoji(emojis: emojiPopView.emojiArray), selectedEmoji: currentEmoji)
        }

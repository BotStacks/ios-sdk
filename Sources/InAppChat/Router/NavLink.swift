...
struct NavLink<Content>: View where Content: View {
  ...
  init(to path: String, replace: Bool = false, @ViewBuilder content: @escaping () -> Content) {
  ...
  var active: Bool {
    ...
    return exact ? navigator.path == absolutePath : navigator.path.starts(with: absolutePath)
  }

  var body: some View {
    Button(action: {
      print("Button ON Press ", path)
      onPressed()
    }) {
      content(active)
    }

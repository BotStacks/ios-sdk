...
public typealias InAppChatAPIChallengeHandler = (
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
  ...
  override open func execute(
    _ apiResponseQueue: DispatchQueue = InAppChatAPI.apiResponseQueue,
    ...
    case is URL.Type:
      do {
        ...
        if let headerFileName = getFileName(
          fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String)
        {
    ...
    default:
      ...
      guard let unwrappedData = data, !unwrappedData.isEmpty else {
        if let E = T.self as? ExpressibleByNilLiteral.Type {
          completion(
            .success(
              Response(response: httpResponse, body: E.init(nilLiteral: ()) as! T, bodyData: data)))
        } else {
          completion(
            .failure(
              ErrorResponse.error(
